...
-   **Maze Generator Defaults (DAW Flow)**: By default, the `MazeGenerator` enforces a Left-to-Right / Top-to-Bottom directional bias for generated mazes to mimic DAW signal flow (Spawns prefer Top/Left, Exits prefer Bottom/Right).
...
-   **Manual Generation Overrides**: Level designers can check "Use Custom Spawns" or "Use Custom Goal" in the `MazeGenerator` inspector and define exact Vector2i coordinates in the Preview Output to bypass the procedural placement logic. Note that very aggressive manual placements with complex constraint settings rely on the generator's internal **2000ms timeout relaxation loop** to guarantee a valid path is found before lowering quality thresholds and emitting a result.

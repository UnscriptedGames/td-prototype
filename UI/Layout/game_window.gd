...
var _meter_noise_offset_left: float = 0.0
var _meter_noise_offset_right: float = 0.0

# Jitter State
var _noise_target_common: float = 0.0
var _noise_value_common: float = 0.0
var _noise_target_difference: float = 0.0
var _noise_value_difference: float = 0.0
...
func _process(delta: float) -> void:
	...
	var target_value: float = _target_damage_value
	...
	if _current_context == ContextMode.STUDIO:
		...
		if is_instance_valid(player):
			...
			if maximum_allocation_points > 0.0:
		...
		if GameManager.max_peak > 0:
			target_value = (GameManager.current_peak / GameManager.max_peak) * 100.0
		else:
			target_value = 0.0

	# 1. Update Noise (simulate live signal jitter for "analogue" feel)
	var maximum_value: float = gauge_l.max_value
	var noise_amplitude_value: float = maximum_value * JITTER_AMPLITUDE

	# Common signal — master jitter shared between L and R
	if abs(_noise_value_common - _noise_target_common) < 0.05:
		_noise_target_common = randf_range(-1.0, 1.0)

	# Difference signal — stereo width jitter
	if abs(_noise_value_difference - _noise_target_difference) < 0.05:
	...
	var common_offset: float = _noise_value_common * noise_amplitude_value
	var differential_offset: float = (
		_noise_value_difference * noise_amplitude_value * STEREO_SEPARATION
	)

	_meter_noise_offset_left = common_offset + differential_offset
	_meter_noise_offset_right = common_offset - differential_offset
...
func _exit_tree() -> void:
	...
	if (
		is_instance_valid(minimize_button)
		and minimize_button.pressed.is_connected(_on_minimize_pressed)
	):
		minimize_button.pressed.disconnect(_on_minimize_pressed)
	if (
		is_instance_valid(maximize_button)
		and maximize_button.pressed.is_connected(_on_maximize_pressed)
	):
		maximize_button.pressed.disconnect(_on_maximize_pressed)
	if is_instance_valid(close_button) and close_button.pressed.is_connected(_on_close_pressed):
...
func _setup_transport() -> void:
	...
	if restart_button:
		restart_button.pressed.connect(_on_restart_button_requested)
...
func _setup_signal_connections() -> void:
	...
	if volume_slider:
		...
		var volume_decibels: float = AudioServer.get_bus_volume_db(master_bus_index)
		volume_slider.value = db_to_linear(volume_decibels) * 100.0
...
func _setup_input_propagation() -> void:
	...
	if viewport_container:
		...
		if viewport_container.get_child_count() > 0:
			var viewport_instance: SubViewport = viewport_container.get_child(0) as SubViewport
			assert(viewport_instance != null)
...
func _update_peak_hold(target_value: float, is_left: bool) -> void:
	...
	# To place the line correctly, map target_value (-5 to 100) to 0.0 - 1.0 physical width.
	if is_instance_valid(line) and is_instance_valid(line.get_parent()):
		var current_width: float = line.get_parent().size.x - line.size.x
		if current_width > 0:
			var percentage: float = clamp((target_value + 5.0) / 105.0, 0.0, 1.0)
			line.position.x = current_width * percentage
...
func _on_restart_button_requested() -> void:
...
func _on_tower_selected(tower: TemplateTower) -> void:
	...
	if _tower_inspector and _tower_inspector.has_method("set_tower"):
		...
		if is_instance_valid(tower):
			...
			var final_screen_position: Vector2 = viewport_local_position + current_container_offset

			# Calculate map coordinates for inspector grid-side logic
			var map_coordinates: Vector2i = Vector2i(-1, -1)
			if _build_manager and is_instance_valid(_build_manager.path_layer):
				...
				map_coordinates = layer.local_to_map(tower_local_to_layer)

			_tower_inspector.update_anchor(viewport_size, final_screen_position, map_coordinates)
...
func _on_peak_changed(current_peak: float, maximum_peak: float) -> void:
	...
	if maximum_peak > 0.0 and is_instance_valid(gauge_l):
		var true_percentage: float = min(100.0, (current_peak / maximum_peak) * 100.0)
		_target_damage_value = true_percentage

		if is_instance_valid(integrity_label):
			var distortion_percentage: int = max(0, floor(true_percentage))
			integrity_label.text = "%d%%" % distortion_percentage
...
func _on_volume_changed(value: float) -> void:
	...
	var master_bus_index: int = AudioServer.get_bus_index("Master")
	AudioServer.set_bus_volume_db(master_bus_index, decibel_value)

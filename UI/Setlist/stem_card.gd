...
func update_state(result: StemResult) -> void:
	...
	match result.status:
		StemResult.StemStatus.LOCKED:
			...
			label_quality.text = ""
			_apply_theme_variation("StemCardLocked", "StemCardLabelLocked")
			mouse_default_cursor_shape = Control.CURSOR_ARROW
		StemResult.StemStatus.AVAILABLE:
			label_status.text = "AVAILABLE"
			label_quality.text = ""
			quality_select.visible = false
			_apply_theme_variation("StemCardAvailable", "StemCardLabelAvailable")
			mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND
		StemResult.StemStatus.COMPLETED:
			label_status.text = "COMPLETED"
			label_quality.text = _quality_text(result.quality)
			label_quality.add_theme_color_override("font_color", _quality_colour(result.quality))
			_populate_quality_dropdown(result)
			quality_select.visible = true
			_apply_theme_variation("StemCardCompleted", "StemCardLabelCompleted")
			mouse_default_cursor_shape = Control.CURSOR_POINTING_HAND
...
func _populate_quality_dropdown(result: StemResult) -> void:
	...
	if best_q <= StemResult.StemQuality.AVERAGE and best_q != StemResult.StemQuality.NONE:
		...
		quality_select.set_item_metadata(
			quality_select.item_count - 1, StemResult.StemQuality.AVERAGE
		)
	...
	if best_q <= StemResult.StemQuality.ABOMINATION and best_q != StemResult.StemQuality.NONE:
		...
		quality_select.set_item_metadata(
			quality_select.item_count - 1, StemResult.StemQuality.ABOMINATION
		)
...
## Updates the theme variation for the card and its labels.
func _apply_theme_variation(card_var: String, label_var: String) -> void:
	theme_type_variation = card_var

	for label in [label_stem_name, label_instrument, label_status, label_quality, label_preview]:
		if is_instance_valid(label):
			label.theme_type_variation = label_var

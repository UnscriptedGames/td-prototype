...
func test_get_total_allocation_cost_empty() -> void:
	...
	assert_eq(
		player_data.get_total_allocation_cost(),
		0,
		"Empty player data should have 0 allocation cost."
	)
...
func test_get_total_allocation_cost_with_towers() -> void:
	...
	assert_eq(
		player_data.get_total_allocation_cost(),
		28,
		"Total cost should include all towers with stock."
	)
...
func test_get_total_allocation_cost_mixed() -> void:
	...
	assert_eq(
		player_data.get_total_allocation_cost(),
		18,
		"Total cost should sum relics, buffs, and towers."
	)
...
func test_update_total_cost_manual_invalidation() -> void:
	...
	assert_eq(
		player_data.total_cost,
		15,
		"total_cost should be updated after calling update_total_cost()."
	)
...
func test_tower_slots_initialization() -> void:
	var player_data: PlayerData = PlayerData.new()
	assert_eq(
		player_data.tower_slots.size(),
		PlayerData.TOWER_SLOT_COUNT,
		"Tower slots should be pre-sized."
	)
	for slot_index in range(PlayerData.TOWER_SLOT_COUNT):
		assert_null(
			player_data.tower_slots[slot_index],
			"Tower slot %d should be null initially." % slot_index
		)

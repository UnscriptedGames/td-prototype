...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton and event.button_index == MOUSE_BUTTON_LEFT:
		...
		var grid_column: int = int(floor(local_position.x / tile_size.x))
		var grid_row: int = int(floor(local_position.y / tile_size.y))
		var coordinates: Vector2i = Vector2i(grid_column, grid_row)

		# Check if hidden
		if _hidden_cells.has(coordinates):
			return

		# Check bounds
		if (
			coordinates.x >= 0
			and coordinates.x < grid_width
			and coordinates.y >= 0
			and coordinates.y < grid_height
		):
			if event.pressed:
				_animate_pad(coordinates, 1.0)
				_randomize_pad_color(coordinates)
				_current_pressed_pad = coordinates
...
func _draw() -> void:
	...
	for column_index: int in range(grid_width):
		for row_index: int in range(grid_height):
			var coordinates: Vector2i = Vector2i(column_index, row_index)
			...
			if _pad_colors.has(coordinates):
				tile_color = _pad_colors[coordinates]

			# 2. Determine Scale & Animation state
			var cell_center: Vector2 = (
				(Vector2(coordinates) * tile_size) + (tile_size / 2.0)
			)
			var base_size := Vector2(28, 28)

			var cell_scale_multiplier: float = _cell_scales.get(coordinates, 1.0)
			var press_ratio: float = _pressed_states.get(coordinates, 0.0)
			...
			var draw_size: Vector2 = (
				base_size * cell_scale_multiplier * press_scale
			)

			# Shift pad down slightly when pressed
			var vertical_offset: float = lerpf(0.0, 4.0, press_ratio)

			var draw_position: Vector2 = (
				cell_center - (draw_size / 2.0) + Vector2(0, vertical_offset)
			)
			var face_rect: Rect2 = Rect2(draw_position, draw_size)
...
func animate_hide_cell(coords: Vector2i, duration: float = 0.5) -> void:
	...
	tween.tween_method(
		func(current_scale: float) -> void:
			_cell_scales[coords] = current_scale
			queue_redraw(),
		start_scale,
		0.0,
		duration
...
func _setup_material() -> void:
	...
	var shader: Shader = load("res://Stages/_Components/matte_surface.gdshader")
	if shader:
...
func _animate_pad(coords: Vector2i, target_value: float) -> void:
	...
	var start_value: float = _pressed_states.get(coords, 0.0)

	tween.tween_method(
		func(current_press_ratio: float) -> void:
			_pressed_states[coords] = current_press_ratio
			queue_redraw(),
		start_value,
		target_value,
